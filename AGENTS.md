...
AI 코딩 에이전트(Roo Code, Claude Code, OpenCode)의 설정·커맨드·스킬·규칙을 심볼릭 링크로 중앙 관리하는 dotfiles 저장소.
...
- 스킬 파일은 `skills/<스킬명>/SKILL.md` 경로를 따르라. Roo Code, Claude Code, OpenCode에서 인식하는 구조.
- OpenCode 커맨드는 `~/.config/opencode/commands` 링크를 통해 `commands/`를 공유하고, 스킬은 `~/.config/opencode/skills` 및 `~/.agents/skills` 경로로 공유한다.
...
├── .opencode/                    # OpenCode 설정 (opencode.json)
